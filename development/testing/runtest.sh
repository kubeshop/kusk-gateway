...
function get_envoy_logs() {
    ...
    info "Retrieving Envoy pod Agent logs for you with filtered out INFO to check for any errors"
    kubectl logs -l app.kubernetes.io/component=envoy -n ${NAMESPACE} -c agent --tail=100 | sed '/INFO/d'
}
