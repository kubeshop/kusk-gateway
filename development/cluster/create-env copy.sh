...
if [ -n "${DEBUG}" ]; then
  DEBUG_LOG="--logtostderr=true"
fi

echo "========> creating cluster..."
minikube start --profile kgw --addons=metallb
# --wait-timeout 16m --wait all
#  --preload=false
# --feature-gates="LocalStorageCapacityIsolation=false"
# minikube start --profile kgw "${DEBUG_LOG}" --addons=metallb --driver=podman --container-runtime=crio-o
# minikube start --profile kgw "${DEBUG_LOG}" --addons=metallb --driver=podman --container-runtime=cri-o
# minikube start --profile kgw "${DEBUG_LOG}" --addons=metallb --driver=podman --container-runtime=containerd
# minikube start --profile kgw "${DEBUG_LOG}" --addons=metallb --driver=docker --container-runtime=cri-o
